...
class HDFMapDigiLeCroy180E(HDFMapDigiTemplate):
    ...
    def deduce_config_active_status(self, config_name: str) -> bool:
        """
        The LeCroy scope does not utilize the concept of configurations,
        so its recordings is always the active "configuration."
        """
        return True

    def _find_adc_connections(self) -> Tuple[Tuple[int, Tuple[int, ...], Dict[str, Any]], ...]:
        """
        ...
        channel_dset_names = ["Channel1", "Channel2", "Channel3", "Channel4"]
        chs = []
        for ch, ch_name in zip([1, 2, 3, 4], channel_dset_names):
            if ch_name not in config_group:
                # channel dataset does not exist
                continue

            if ch_name not in config_group["Headers"]:
                # channel header dataset does not exist
                continue

            dset = config_group[ch_name]
    ...
    def _adc_info_first_pass(self) -> Tuple[Tuple[int, Tuple[int, ...], Dict[str, Any]], ...]:
    ...
    def _adc_info_second_pass(self) -> Tuple[Tuple[int, Tuple[int, ...], Dict[str, Any]], ...]:
        """
        ...
        for ch in channels:
            # get datasets
            names = [
                self.construct_dataset_name(board, ch),
                self.construct_header_dataset_name(board, ch),
            ]
            for dset_name in names:
                if dset_name not in self.group:
                    why = (
                        f"HDF5 structure unexpected...dataset '{dset_name}' "
                        f"not found for board {board} and channel {ch}"
        ...
        if len(new_chs) == 0:
            ...
            raise HDFMappingError(
                ...
                why=f"HDF5 structure unexpected...'Board number {board}' "
        ...
        for ch in new_chs:
            # -- examine dataset --
            dset_name = self.construct_dataset_name(board, ch)
        ...
        if len(new_chs) == 0:
            raise HDFMappingError(
                self.info["group path"],
                why=f"HDF5 structure unexpected...'Board number {board}' does not "
                    f"define any valid channel numbers...not adding to "
                    f"`configs` dict",
            )
    ...
    def _build_configs(self):
        ...
        if "Headers" not in subgroup_names:
            raise HDFMappingError(
                self.info["group path"],
                why="Digitizer is missing the 'Headers' group at "
