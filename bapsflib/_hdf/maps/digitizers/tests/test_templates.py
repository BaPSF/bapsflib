...
class TestDigiTemplates(ut.TestCase):
    ...
    def map_device(self, group: h5py.Group) -> HDFMapDigiTemplate:
        ...
        dummy_map = \
            type("%sDummy" % self.MAP_CLASS.__name__,
                 (self.MAP_CLASS,), new_dict)
    ...
    def test_deduce_config_active_status(self):
        ...
        self.assertFalse(
            self.map.deduce_config_active_status(config_name))
        ...
        self.assertTrue(
            self.map.deduce_config_active_status(config_name))
        self.assertFalse(
            self.map.deduce_config_active_status('hello'))

        # clean up
        self.group.clear()

    @mock.patch.object(HDFMapDigiTemplate, 'configs',
    ...
    def test_get_adc_info(self, mock_configs):
        ...
        with self.assertWarns(UserWarning):
            adc_info = _map.get_adc_info(brd, ch,
        ...
        ch = _map.configs[config_name][adc][0][1][0]
        with self.assertRaises(ValueError):
            _map.get_adc_info(brd, ch,
                              config_name=config_name, adc=adc)
    ...
    def assertDigiTemplate(self,
        ...
        #
        attrs = ('_build_configs', 'active_configs', 'configs',
                 'construct_dataset_name',
                 'construct_header_dataset_name',
                 'deduce_config_active_status', 'device_adcs',
                 'device_name', 'get_adc_info', 'group', 'info')
        for attr in attrs:
        ...
        #
        abs_methods = ('_build_configs', 'construct_dataset_name',
                       'construct_header_dataset_name')
        for method in abs_methods:
            self.assertEqual(_map.__class__.__dict__[method](),
                             NotImplemented)
