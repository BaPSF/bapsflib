...
class TestHDFMap(ut.TestCase):
    ...
    def test_all_devices_in_one_group(self):
        ...
        _map = self.map_file(self.f, msi_path='Raw data + config',
                             digitizer_path='Raw data + config',
                             control_path='Raw data + config')
        ...
        self.assertEqual(sorted(_map.unknowns),
    ...
    def test_empty_file(self):
        ...
        _map = self.map_file(self.f, msi_path='',
                             digitizer_path='', control_path='')
        self.assertHDFMapBasics(_map, self.f)
        self.assertEqual(_map.controls, {})
        self.assertEqual(_map.digitizers, {})
        self.assertEqual(_map.msi, {})
        self.assertEqual(sorted(_map.unknowns),
                         sorted(['/MSI', '/Raw data + config']))
    ...
    def test_main_digitizer(self):
        ...
        _map = self.map_file(self.f, msi_path='MSI',
                             ...
                             control_path='Raw data + config')
        self.assertHDFMapBasics(_map, self.f)
        self.assertEqual(_map.main_digitizer,
                         _map.digitizers['SIS 3301'])
    ...
    def test_msi_mapping_attachment(self):
        ...
        with self.assertWarns(UserWarning):
            _map = self.map_file(self.f, msi_path='Not a MSI path',
                                 digitizer_path='Raw data + config',
                                 control_path='Raw data + config')
            self.assertHDFMapBasics(_map, self.f)
            self.assertEqual(_map.msi, {})
    ...
    def assertHDFMapBasics(self, _map, _file):
        ...
        self.assertIsInstance(_map.main_digitizer,
                              (type(None), HDFMapDigiTemplate))
        ...
        self.assertTrue(all(isinstance(val, str)
                            for val in _map.unknowns))
