...
class LaPDOverview(HDFOverview):
    ...
    def report_general(self):
        ...
        print("LaPD version: {}".format(self._file.info["lapd version"]))
        print("Investigator: {}".format(self._file.info["investigator"]))
        print("Run Date:     {}".format(self._file.info["run date"]))

        # exp. and run structure
        print("\nExp. and Run Structure:")
        print("  (set)  {}".format(self._file.info["exp set name"]))
        print("  (exp)  +-- {}".format(self._file.info["exp name"]))
        print("  (run)  |   +-- {}".format(self._file.info["run name"]))
