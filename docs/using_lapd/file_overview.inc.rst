...
The `~lapdoverview.LaPDOverview` class provides a set of tools (see
:numref:`f_overview_methods`) to report the results of the file mapping
generated by `~mapper.LaPDMapper`. An instance of
`~lapdoverview.LaPDOverview` is bound to
`~bapsflib.lapd._hdf.file.File.File` as the
:attr:`~bapsflib.lapd._hdf.file.File.File.overview` attribute and will
report the current status of the mapping object.
...
location.  The report is divided into three blocks:
    ...
    #. Discovery Report
        ...
        * This block gives a brief report on what devices the
          `~mapper.LaPDMapper` class discovered.
